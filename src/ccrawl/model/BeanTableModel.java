...
public class BeanTableModel<T> extends RowTableModel<T>
{
	...
	private static Map<Class, Class> primitives = new HashMap<Class, Class>(10);
        private String[] methodNamesToExclude = { "lastLineNumber", "matchStartIndex", "matchEndIndex", "match", "newline"};
	...
	private List<ColumnInformation> columns = new ArrayList<ColumnInformation>();
        private HashMap<String, String> columnNameMapping = new HashMap<String, String>();
	...
	{
		this(beanClass, beanClass, new ArrayList<T>(), null, null);
	}
	 ...
	 *                    will be displayed in the model
	 */
	public BeanTableModel(Class beanClass, HashMap columnNameMapping, List<String> columnNamesSorted)
	{
		this(beanClass, beanClass, new ArrayList<T>(), columnNameMapping, columnNamesSorted);
	}
	...
	{
		this(beanClass, ancestorClass, new ArrayList<T>(), null, null);
	}
	...
	{
		this(beanClass, beanClass, modelData, null, null);
	}
	 ...
	 *  @param modelData      the data of the table
	 */
	public BeanTableModel(Class beanClass, Class ancestorClass, List<T> modelData, HashMap columnNameMapping, List<String> columnNamesSorted)
	{
		super( beanClass );
                
                if(columnNameMapping != null)
                    this.columnNameMapping = columnNameMapping;
                if(columnNamesSorted != null)
                    this.columnNamesSorted = columnNamesSorted;
	...
	private void buildColumnInformation(Method theMethod, String theMethodName)
	{
		...
		if (returnType == null) return;
                ...
                for (int i = 0; i < this.methodNamesToExclude.length; i++) {
                    if (this.methodNamesToExclude[i].compareToIgnoreCase(theMethodName) == 0) {
                        return;
                    }
                }
	...
	{
                if(columnNamesSorted.size() > 0){
                    List<ColumnInformation> sortedColumns = new ArrayList<ColumnInformation>();
                    for(String columnName : columnNamesSorted){
                        for(ColumnInformation aColumn : columns){
                            if(aColumn.getName().equalsIgnoreCase(columnName)){
                                sortedColumns.add(aColumn);
                                break;
                            }
                        }                            
                    }            
	...
	private class ColumnInformation implements Comparable<ColumnInformation>
	{
		...
		public ColumnInformation(String name, Class returnType, Method getter, Method setter)
		{
			...
			this.setter = setter;
                        ...
                        if(!columnNameMapping.isEmpty() && columnNameMapping.containsKey(name))
                            this.name = columnNameMapping.get(name);                            
		}
